...
pub(crate) fn generate(typename: &str, dataitem: &DataItem, is_a2l: bool) -> TokenStream {
    ...
    match &dataitem.basetype {
        ...
        BaseType::Struct { structitems } => {
            result.extend(generate_block_data_structure_generic(
                typename,
                structitems,
                true,
                false,
        ...
        BaseType::Block {
            ...
            result.extend(generate_block_data_structure(
                typename,
                blockitems,
                *is_block,
                *used_in_list,
...
fn generate_block_data_structure(
    typename: &str,
    ...
    match typename {
        ...
        _ => generate_block_data_structure_generic(
            typename,
            structitems,
            false,
            is_block,
...
fn generate_block_data_structure_generic(
    typename: &str,
    structitems: &[DataItem],
    is_struct: bool,
    is_block: bool,
    ...
    for (idx, item) in structitems.iter().enumerate() {
        definitions.push(generate_struct_item_definition(
            item,
            idx == 0,
            used_in_list,
        ));
...
fn generate_struct_item_definition(
    item: &DataItem,
    is_first: bool,
    used_in_list: bool,
) -> TokenStream {
    ...
    match &item.basetype {
        ...
        BaseType::TaggedStructRef => {
            panic!("TaggedStructRef should have been resolved in the data structure fixup phase");
        }
        ...
        | BaseType::TaggedStruct { tsitems: tgitems } => {
            ...
            for tgitem in tgitems {
                ...
                if tgitem.repeat {
                    if tgitem.is_named {
                        curr_def.extend(quote! {pub #tgitemname: ItemList<#typename>});
                    } else {
        ...
        _ => {
            ...
            // if the first item is an ident called "name", then it is the name of a named block, and should not be pub
            // The name should be accessed though get_name() and set_name()
            if is_first && used_in_list && itemname == "name" {
                def.extend(quote! {pub(crate) #itemname: #typename});
            } else {
...
fn generate_block_data_structure_constructor(
    typename: &str,
    structitems: &[DataItem],
    is_struct: bool,
    is_block: bool,
        ...
        match &item.basetype {
            ...
            | BaseType::TaggedStruct {
                ...
                for tgitem in taggeditems {
                    ...
                    if tgitem.repeat {
                        // repeating items are either a Vec for unnamed blocks or a ItemList for named blocks
                        // this allows the named blocks to be indexed by their position and their name
                        if tgitem.is_named {
                            fieldinit.push(quote! {#tgitemname: ItemList::default()});
                        } else {
...
fn generate_block_data_structure_partialeq(
    typename: &str,
    structitems: &[DataItem],
) -> TokenStream {
    let typeident = format_ident!("{}", typename);
...
fn generate_block_data_structure_trait_name(
    typename: &str,
    structitems: &[DataItem],
) -> TokenStream {
    let typeident = format_ident!("{}", typename);

    if !structitems.is_empty()
        && structitems[0].basetype == BaseType::Ident
        && structitems[0].varname == Some("name".to_string())
    {
        quote! {
            ...
            impl A2lObjectNameSetter for #typeident {
